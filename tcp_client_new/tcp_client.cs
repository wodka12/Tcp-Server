...
#if true
                    //ipep = new IPEndPoint(IPAddress.Parse("172.23.24.11"), 4000);
                    //ipep = new IPEndPoint(IPAddress.Parse("54.199.247.174"), 4000);
                    ...
                    if (0 < ns.Read(bByte, 0, Marshal.SizeOf(typeof(_User))))
                    {
                        ...
                        switch (us_header)
                        {
                            ...
                            case (ushort)e_Packet_header.P_BROAD_CAST_MSG:
                                ...
                                Array.Copy(bByte, arr_idx, bmsg, 0, 4);
                                rcv_str = Encoding.Default.GetString(bmsg);   // byte -> string
                                receive_data = "Notify! user joined number [" + id.ToString() + "]" + ": " + rcv_str;
                                break;
