...
/* 2016.05.23 */
void ClientManager::recv_packet_user_join_room(int user_fd)
{
...
int ClientManager::packet_lobby_proc(ObjectUser::User* info)
{
	int ret = 0;
	...
	{
	case P_S_JOIN_ROOM:
		recv_packet_user_join_room(info->id);
		break;
	case P_S_QUIT_ROOM:
...
int ClientManager::Recv_Client_Packet(SOCKETINFO* socket_info, BYTE* packet)
{
	...
	if (pObjectUser != NULL) {
		...
		case P_LOBBY_MSG:
			...
			packet_lobby_proc(&pObjectUser->sUser_info);
			break;
		case P_MAX:
			break;
		}
